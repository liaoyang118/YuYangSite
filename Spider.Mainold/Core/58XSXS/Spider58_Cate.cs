...
namespace Spider.Main.Core
{
    ...
    public class Spider58_Cate
    {
        ...
        /// </param>
        public Spider58_Cate(MainForm form, string domain, string logTaskHead, string pageFormat, string cateName, int totalPage, int pageNums, int fictionNums, int nums, int UpdateGrowNums)
        {
            main = form;
            Domain = domain;
        ...
        private void PageThreadRun(object obj)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    ...
                    if (mList.Count > 0)
                    {
                        main.WriteSuccessLog(string.Format("{0}:分类【{1}】,获取第【{2}】页数据【{3}】条", LogTaskHead, _currentCateName, pageIndex, mList.Count - 1));
                        //跳过第一条
