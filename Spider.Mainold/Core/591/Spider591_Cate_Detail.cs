    ...
    public class Spider591_Cate_Detail
    {
        ...
        private void FictionThreadRun(object obj)
        {
            ...
            try
            {
                if (fictionIndex < _watingGetFictionList.Count)
                {
                    ...
                    {
                        main.WriteErrorLog(string.Format("{0}:分类【{1}】,获取第【{2}】页数据，视频【{3}】出错，详情：【{3}】", LogTaskHead, _currentCateName, _currentPage, fInfo.Title, ex.Message));
                    }
                }
            }
            catch (Exception ex)
        ...
        private string GetCoverImage(VideoInfo fInfo, string detailUrl)
        {
            ...
            try
            {
                //判断是否存在
                bool isExist = false;
                VideoInfo isExistInfo = VideoInfoService.Select(string.Format("where title='{0}'", fInfo.Title)).FirstOrDefault();
                if (isExistInfo != null)
                {
                    isExist = true;
                    fInfo = isExistInfo;
                ...
                main.WriteSuccessLog(string.Format("{0}:分类【{1}】,视频【{2}】获取封面地址：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, detailUrl));
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    ...
                    if (m != null)
                    {
                        ...
                        Match srcM = Regex.Match(m.Value, @"var." + paraJs + @"[\s]*=[\s]*(?<jsonStr>{([^;]+));");
                        ...
                        if (srcM != null)
                        {
                            ...
                            JArray arr = (JArray)obj["mediaDefinitions"];
                            if (arr.Count > 0)
                            {
                        ...
                        if (!string.IsNullOrEmpty(videoSrc))
                        {
                            ...
                            main.WriteSuccessLog(string.Format("{0}:分类【{1}】,591视频【{2}】获取视频,视频地址：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, videoSrc));
                            if (!isExist)
                            {
                                #region 下载网络视频到本地，生成本地路径
                                string error;
                                string videoServerSrc = web.CaptureRemoteVedio(videoSrc, out error, "VideoUpload", percent =>
                                {
                                    main.WriteNotifyLog(string.Format("{0}:分类【{1}】,591视频【{2}】获取进度：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, percent));
                                });
                                if (string.IsNullOrEmpty(videoServerSrc))
                                {
                                    main.WriteErrorLog(string.Format("{0}:分类【{1}】,591视频【{2}】获取封面出错：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, error));
                                }
                                else
                                {
                                    //fInfo.CarsCoverSrc = coverImageSrc;
                                    main.WriteSuccessLog(string.Format("{0}:分类【{1}】,591视频【{2}】获取视频成功：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, videoServerSrc));
                                }
                                #endregion
                            }
                        }
                        ...
                        {
                            main.WriteErrorLog(string.Format("{0}:分类【{1}】,591视频【{2}】,未匹配到Video元素。", LogTaskHead, _currentCateName, fInfo.Title));
                        }
                    }
            ...
            {
                main.WriteErrorLog(string.Format("{0}:分类【{1}】,591视频【{2}】获取视频出错：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, ex.Message));
            }
            return identityId;
        }
