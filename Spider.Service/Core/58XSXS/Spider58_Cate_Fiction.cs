...
namespace Spider.Service
{
    public class Spider58_Cate_Fiction
    {
        ...
        /// </param>
        public Spider58_Cate_Fiction(IWriteContent form, string domain, string logTaskHead, List<KeyValuePair<Fiction, string>> watingInsertList, string cateName, int nums, int fictionNums, int currentPage, int UpdateGrowNums)
        {
            main = form;
            Domain = domain;
        ...
        public void GetFictionByPage()
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    {
                        IsComplete = true;
                        main.WriteNotifyContet(string.Format("{0}:分类【{1}】，第【{2}】页小说列表获取完毕", LogTaskHead, _currentCateName, _currentPage));
                    }
                    break;
                }
            }
        ...
        private void FictionThreadRun(object obj)
        {
            ...
            try
            {
                ...
                {
                    main.WriteErrorContet(string.Format("{0}:分类【{1}】,获取第【{2}】页数据，小说【{3}】出错，详情：【{3}】", LogTaskHead, _currentCateName, _currentPage, fInfo.Title, ex.Message));
                }
            }
            catch (Exception ex)
            {
                main.WriteErrorContet(string.Format("{0}:分类【{1}】,获取小说出错，可能是索引超出。当前索引【{2}】，总长度：【{3}】:详情：【{4}】", LogTaskHead, _currentCateName, fictionIndex, _watingGetFictionList.Count, ex.Message));
            }
        }
        ...
        private int GetCoverImage(Fiction fInfo, string fictionUrl)
        {
            ...
            try
            {
                ...
                main.WriteSuccessContet(string.Format("{0}:分类【{1}】,小说【{2}】获取封面,主页地址：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, fictionUrl));
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        main.WriteSuccessContet(string.Format("{0}:分类【{1}】,小说【{2}】获取封面,封面地址：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, imageSrc));
                        if (!isExist)
                        {
                            ...
                            {
                                main.WriteErrorContet(string.Format("{0}:分类【{1}】,小说【{2}】获取封面出错：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, error));
                            }
                            else
                            {
                    ...
                    {
                        main.WriteErrorContet(string.Format("{0}:分类【{1}】,小说【{2}】,未匹配到封面元素。", LogTaskHead, _currentCateName, fInfo.Title));
                    }
                    ...
                    if (string.IsNullOrEmpty(fInfo.Intro))
                    {
                        ...
                        {
                            main.WriteErrorContet(string.Format("{0}:分类【{1}】,小说【{2}】,未匹配到内容简介。", LogTaskHead, _currentCateName, fInfo.Title));
                        }
                    }
                    ...
                    if (identityId > 0)
                    {
                        ...
                        if (Listmatch.Success)
                        {
                            ...
                            {
                                main.WriteErrorContet(string.Format("{0}:分类【{1}】,小说【{2}】,未匹配到列表页面地址", LogTaskHead, _currentCateName, fInfo.Title));
                            }

                        }
                        else
                        {
                            main.WriteErrorContet(string.Format("{0}:分类【{1}】,小说【{2}】,未匹配到页面列表元素。", LogTaskHead, _currentCateName, fInfo.Title));
                        }
                    }
                }
                else
                {
                    main.WriteErrorContet(string.Format("{0}:未获取到获取小说主页Dom元素", LogTaskHead));
                }

            }
            catch (Exception ex)
            {
                main.WriteErrorContet(string.Format("{0}:分类【{1}】,小说【{2}】获取封面出错：【{3}】", LogTaskHead, _currentCateName, fInfo.Title, ex.Message));
            }
            return identityId;
        }
        ...
        private void Watch(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            //        {
            //            main.WriteNotifyContet(string.Format("{0}:分类【{1}】，页【{2}】,【{3}】终止。", LogTaskHead, _currentCateName, _currentFictionIndex, excuteThreads[i].Name));
