...
namespace Spider.Main.Core
{
    public class SpiderCars_Cate
    {
        ...
        /// </param>
        public SpiderCars_Cate(MainForm form, string domain, string logTaskHead, string pageFormat, string cateName, int totalPage, int pageNums, int fictionNums, int nums, int UpdateGrowNums)
        {
            main = form;
            Domain = domain;
            ...
            //分页数据按照数据条数起止
            //获取 rpp
            pageInterval = Regex.Match(pageFormat, @"rpp=(?<rpp>[\d]+)").Groups["rpp"].ToString().ToInt32(0);
        ...
        {
            List<KeyValuePair<Mongo_CarsInfo, string>> watingGetCarsList = new List<KeyValuePair<Mongo_CarsInfo, string>>();

            int pageIndex = (int)obj;
            try
            {
                string pageUrl = string.Format(_pageFormat, pageIndex * pageInterval);
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    ...
                    HtmlNodeCollection items = htmlDoc.DocumentNode.SelectNodes("//*[@id=\"mlp-listings\"]/div[1]/div[2]/div[@class=\"listingCard\"]");
                    ...
                    if (items != null && items.Count > 0)
                    {
                        ...
                        {
                            detailUrl = Domain + item.SelectSingleNode("child::div[1]/a[@data-linkname=\"md-ymmt\"]").Attributes["href"].Value;
                            title = item.SelectSingleNode("child::div[1]/a[1]/h4[1]").InnerText;
                            price = item.SelectSingleNode("child::div[1]/div[1]/div[1]/p[2]").InnerText.Replace("$", "").Replace(",", "").ToInt32(0);

                            //小说对象
                            Mongo_CarsInfo cInfo = new Mongo_CarsInfo();
                            cInfo.AreaId = string.Empty;
                            cInfo.Body = string.Empty;
                            cInfo.BodyType = string.Empty;
                            cInfo.BuildDate = string.Empty; //生产日期
                            cInfo.CarsCateId = string.Empty;
                            cInfo.CarsCoverSrc = string.Empty;
                            cInfo.Colour = string.Empty;
                            cInfo.ComplianceDate = string.Empty;//上牌时间
                            cInfo.DriveType = string.Empty;
                            cInfo.Engine = string.Empty;
                            cInfo.FuelEconomyCombined = string.Empty;
                            cInfo.FullName = title;//国外名称
                            cInfo.FullNameCN = string.Empty;//中文名称
                            cInfo.Kilometres = 0;
                            cInfo.Price = price;
                            cInfo.PriceUnti = PriceUnti.美元.ToString();
                            cInfo.ReleaseDate = string.Empty; //发布时间
                            cInfo.Transmission = string.Empty;//变速箱
                            cInfo.TransmissionType = string.Empty;//变速箱类型，自动，手动
                            cInfo.Used = true;//是否新车

                            watingGetCarsList.Add(new KeyValuePair<Mongo_CarsInfo, string>(cInfo, detailUrl));
                        }
                    }
                    else
                    {
                        main.WriteErrorLog(string.Format("{0}:分类【{1}】,未获取到第【{2}】页数据", LogTaskHead, _currentCateName, pageIndex));
                    }
                }
                else
                {
                ...
                {
                    //获取小说
                    SpiderCars_Cate_Detail scf = new SpiderCars_Cate_Detail(main, Domain, LogTaskHead, watingGetCarsList, _currentCateName, _Nums, _fictionNums, pageIndex, _UpdateGrowNums);
