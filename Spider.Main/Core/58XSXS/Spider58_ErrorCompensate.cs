...
namespace Spider.Main.Core
{
    ...
    public class Spider58_ErrorCompensate : ISpider
    {
        ...
        public Spider58_ErrorCompensate(MainForm form, int pageCount)
        {
            main = form;
        ...
        {
            main.WriteSuccessLog(string.Format("{0}:任务开始。", LogTaskHead));
            ...
            if (totalPage > 1)
            {
                for (pageIndex = 2; pageIndex <= totalPage; pageIndex++)
                {
                    ...
                    {
                        main.WriteNotifyLog(string.Format("{0}:退出任务成功。", LogTaskHead), true);
                        break;
                    }
                }
            }

            main.WriteNotifyLog(string.Format("{0}:任务结束。", LogTaskHead), true);
        ...
        private void GetListByPageIndex(int pageIndex, ErrorChapterSearchInfo search, out int rowCount)
        {
            ...
            try
            {
                ...
                main.WriteSuccessLog(string.Format("{0}:开始处理第【{1}】页数据，共【{2}】条。", LogTaskHead, pageIndex, rowCount));
                foreach (ErrorChapter item in list)
                {
                    ...
                    {
                        main.WriteNotifyLog(string.Format("{0}:停止任务成功。", LogTaskHead), true);
                        break;
                    }
                }
            }
            ...
            {
                main.WriteErrorLog(string.Format("{0}:获取数据错误。【{1}】", LogTaskHead, ex.Message));
            }
        }
        ...
        private void GetChapter(ErrorChapter eInfo)
        {
            ...
            try
            {
                if (eInfo.C_C_Id <= 0)
                {
                    ...
                    if (fInfo == null)
                    {
                        main.WriteErrorLog(string.Format("{0}:小说【{1}】已不存在，Id【{2}】", LogTaskHead, eInfo.Title, eInfo.F_Id));
                ...
                if (isExistInfo == null)
                {
                    ...
                    if (!string.IsNullOrEmpty(result))
                    {
                        ...
                        {
                            main.WriteSuccessLog(string.Format("{0}:小说【{1}】,章节【{2}】内文数据成功！", LogTaskHead, eInfo.Title, eInfo.ChapterName));
                        ...
                        {
                            main.WriteErrorLog(string.Format("{0}:小说【{1}】,章节【{2}】，未获取到内文数据", LogTaskHead, eInfo.Title, eInfo.ChapterName));
                            success = false;
                        }
                    }
                    else
                    {
                        main.WriteErrorLog(string.Format("{0}:未获取到小说【{1}】章节【{2}】内文Dom元素", LogTaskHead, eInfo.Title, eInfo.ChapterName));
                        success = false;
                    }
                }
                else
                {
                    main.WriteErrorLog(string.Format("{0}:小说【{1}】，章节【{2}】已存在，Id【{3}】", LogTaskHead, eInfo.Title, eInfo.ChapterName, eInfo.F_Id));
                    success = true;
                }
            }
            catch (Exception ex)
            {
                main.WriteErrorLog(string.Format("{0}:获取小说【{1}】章节【{2}】内文数据错误。【{3}】{4}", LogTaskHead, eInfo.Title, eInfo.ChapterName, eInfo.ChapterUrl, ex.Message));
                success = false;
            }
            finally
            {
