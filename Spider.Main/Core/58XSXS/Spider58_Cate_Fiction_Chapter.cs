...
namespace Spider.Main.Core
{
    ...
    public class Spider58_Cate_Fiction_Chapter
    {
        ...
        public Spider58_Cate_Fiction_Chapter(MainForm form, string domain, string logTaskHead, string cateName, int fid, Fiction fInfo, int nums, int currentPage, int UpdateGrowNums)
        {
            main = form;
            Domain = domain;
        ...
        {
            main.WriteSuccessLog("章节列表页：" + fictionListUrl);
            try
            {
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    ...
                    if (mList.Count > 0)
                    {
                        main.WriteSuccessLog(string.Format("{0}:分类【{1}】,获取【{2}】小说，章节列表数据", LogTaskHead, _currentCateName, _title));
                        //跳过第一条
                    ...
                    {
                        main.WriteSuccessLog(string.Format("{0}:分类【{1}】,小说【{2}】，未获取到章节列表", LogTaskHead, _currentCateName, _title));
                    }
                }
                else
                {
                    main.WriteErrorLog(string.Format("{0}:分类【{1}】,未获取到小说【{2}】章节列表Dom元素", LogTaskHead, _currentCateName, _title));
                }
                //判断是否是已完成抓取状态，如果是，只取最后的50章节
            ...
            {
                main.WriteErrorLog(string.Format("{0}:分类【{1}】,获取小说【{2}】章节列表错误【{3}】", LogTaskHead, _currentCateName, _title, ex.Message));
            }
        ...
        public void ThreadStart()
        {
            ...
            //同时抓取的章节数
            //int totalThreads = 5;
            //if (waitGetContentList.Count < totalThreads)
            //{
            //    totalThreads = waitGetContentList.Count;
            //}
            //Thread t = null;
            //for (int i = 0; i < totalThreads; i++)
            //{
            //    t = new Thread(new ParameterizedThreadStart(ContentThreadRun));
            //    t.IsBackground = true;
            //    t.Name = string.Format("章节线程{0}", _currentIndex);
            //    t.Start(_currentIndex - 1);//为了索引和序号一致
            //    excuteThreads.Add(t);
            ...
            while (true)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        main.WriteNotifyLog(string.Format("{0}:分类【{1}】,小说【{2}】，章节获取完毕", LogTaskHead, _currentCateName, _title), true);
                        //更新当前小说的状态
                    ...
                    {
                        main.WriteErrorLog(string.Format("{0}:分类【{1}】,小说【{2}】，章节获取完毕,插入尾章节时错误:{3}", LogTaskHead, _currentCateName, _title, ex.Message));
                    }
        ...
        private Task CreateNewTask(int currentIndex)
        {
            ...
            //task.ContinueWith((t) =>
            //{
            //    if (currentIndex < waitGetContentList.Count)
            //    {
            //        //当前任务执行完毕，再开新任务
            //        t = CreateNewTask(_currentIndex);
            //        lock (taskLock)
            //        {
            //            //章节序号+1
            //            _currentIndex++;
            ...
            //                IsComplete = true;
            //            }
            //            main.WriteNotifyLog(string.Format("{0}:分类【{1}】,小说【{2}】，章节获取完毕", LogTaskHead, _currentCateName, _title));
            //            //更新当前小说的状态
            //            _fInfo.CompleteState = (int)SiteEnum.CompleteState.完成;
        ...
        private void ContentThreadRun(object obj)
        {
            ...
            try
            {
                ...
                //获取章节内容
                main.WriteSuccessLog(string.Format("{0}:分类【{1}】,第【{4}】页，小说【{2}】，开始获取章节【{3}】数据", LogTaskHead, _currentCateName, _title, chapterTitle, _currentPage));
                ...
                {
                    main.WriteNotifyLog(string.Format("{0}:分类【{1}】,小说【{2}】，章节【{3}】已存在", LogTaskHead, _currentCateName, _title, chapterTitle));
                }

            }
            catch (Exception ex)
            {
                main.WriteErrorLog(string.Format("{0}:分类【{1}】,小说【{4}】，总长度【{5}】，获取章节列表序号【{2}】元素出错，详情：【{3}】", LogTaskHead, _currentCateName, _index, ex.Message + ex.InnerException == null ? "" : ex.Message + ex.InnerException.Message, _title, waitGetContentList.Count));
            }
        ...
        private void GetChapter(Mongo_ChapterList info, string chapterUrl, string title)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    ...
                    {
                        main.WriteSuccessLog(string.Format("{0}:分类【{1}】,获取【{2}】小说，章节【{3}】内文数据成功！", LogTaskHead, _currentCateName, title, string.Format("{0}-{1}", info.ChapterIndex, info.ChapterName)));
                        ...
                        {
                            main.WriteNotifyLog(string.Format("{0}:分类【{1}】,【{2}】小说，批量插入【{3}】条章节数据", LogTaskHead, _currentCateName, title, watingInsertList.Count));
                            try
                            {
                                Mongo_ChapterListService.BatchInsert(watingInsertList, _currentCateId);
                                watingInsertList.Clear();
                            ...
                            {
                                main.WriteErrorLog(string.Format("{0}:分类【{1}】,【{2}】小说，批量插入章节失败", LogTaskHead, _currentCateName, title, watingInsertList.Count));
                            }
                    ...
                    {
                        main.WriteErrorLog(string.Format("{0}:分类【{1}】,小说【{2}】，【{3}】未获取到内文数据", LogTaskHead, _currentCateName, title, string.Format("{0} {1}", info.ChapterIndex, info.ChapterName)));
                    }
                }
                else
                {
                    main.WriteErrorLog(string.Format("{0}:分类【{1}】,未获取到小说【{2}】章节【{3}】内文Dom元素", LogTaskHead, _currentCateName, title, string.Format("{0} {1}", info.ChapterIndex, info.ChapterName)));
                }
            ...
            {
                main.WriteErrorLog(string.Format("{0}:分类【{1}】,获取小说【{2}】章节【{5}】内文数据错误。【{4}】{3}", LogTaskHead, _currentCateName, title, ex.Message, chapterUrl, string.Format("{0} {1}", info.ChapterIndex, info.ChapterName)));
        ...
        private void Watch(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            //    {
            //        //该分类任务执行完毕
            //        main.WriteErrorLog(string.Format("{0}:分类【{1}】小说【{2}】章节全部获取完毕。", LogTaskHead, _currentCateName, _title));
            //        lock (insertLock)
            //        {
            //            //最后批量插入尾数
            //            if (watingInsertList.Count > 0)
            ...
            //            //结束旧线程
            //            //该小说章节全部获取完毕
            //            main.WriteNotifyLog(string.Format("{0}:分类【{1}】小说【{2}】,【{3}】终止。", LogTaskHead, _currentCateName, _title, excuteThreads[i].Name));
