...
namespace DataAccess.Base
{
    ...
    public abstract class AccessBasePartial<T> where T : class
    {
        //抽象属性,批量插入要用
        ...
        public int SqlBulkCopyBatchInsert(IList list, int tableIndex, int batchSize = 100000)
        {
            using (SqlConnection connection = new SqlConnection(ConnectionStr))
            {
                using (SqlBulkCopy sqlBulkCopy = new SqlBulkCopy(ConnectionStr, SqlBulkCopyOptions.UseInternalTransaction))
                {
                    try
                    {
                        ...
                        sqlBulkCopy.DestinationTableName = string.Format(DataTableNameFormat, tableIndex);
